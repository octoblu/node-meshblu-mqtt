...
function Connection(opt){
  ...
  try {
    this.mqttclient = mqtt.createClient(this.options.port || this.options.mqttport || 1883,
                                        this.options.host || this.options.mqtthost || this.options.server || 'meshblu.octoblu.com',
                                        this.mqttsettings);
    ...
    this.mqttclient.on('message', function(topic, data){
      try{
        ...
        if(typeof dataString === 'string' && dataString.indexOf('{') === 0){
          dataString = JSON.parse(dataString);
          if(dataString.topic){
            if(dataString.topic === 'messageAck'){
              var msg = dataString.data;
              ...
              self._handleAckRequest(dataString.topic, dataString.data);
            }
          }else if (dataString.payload){
            self._handleAckRequest('tb', dataString.payload);
          }
        }else{
          //console.log('emitting', typeof data, data);
          self.emit('tb', dataString);
          //self._handleAckRequest('tb', data);
        }
...
Connection.prototype.subscribeText = function(uuid) {
  this.mqttclient.subscribe(uuid + '_tb', {qos: this.options.qos});
  return this;
};
...
Connection.prototype.unsubscribeText = function(data) {
  this.mqttclient.unsubscribe(data.uuid + '_bc');
  return this;
};
